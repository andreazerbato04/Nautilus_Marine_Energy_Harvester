...
<h2>OUR PROTOTYPE</h2>
For our prototype, we put a DC motor for this reason we don't need a bridge rectifire and the capacitor because the voltage is just a DC and not an AC.Another change is the diode, for space reason is put on the negative line. The other part of the circuit remain equal.<br> We built this project and we see that works, in this picture we can see the red led of the first line on, the DC solar panel line, this is because there is sunlight and the panel is charging the batteries. If we put some torque on the DC motor also the red led of the second line start to bright.<br> For the project we glue with the glue gun the component to a cardobard to stabylize the project.<br> We see the solar panel sold with the specific 5 V on peak is a 6V on peak can be an idea use a buck because we there a lot of sunlight the TP4056 can have a high voltage and diced to don't charge.<br><br>
