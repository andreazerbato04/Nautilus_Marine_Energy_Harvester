...
<li><b>Buck-Boost LM2577S LM2596S:</b> stabilize the tension on 5 V, the LM2577S act as a boost converter step up the voltage if is too lower and the LM2596S act as a buck converter step down the voltage if is too high. </li>
<li><b>Solar Panel 5V 5W:</b> generate DC voltage when is exposed to sunlight. </li>
<li><b>Schottky diode 1N5822:</b> prevent the return of the current on the solar panel with a low voltage drop.</li>
...
<img width="1876" height="797" alt="image" src="https://github.com/user-attachments/assets/317325f4-4d95-4016-8b85-7f95bdfe9d20" /><br><br>
 ...
 The circuit was created using fritzing. You can find the file in this folder and download it or use this <a href="https://github.com/andreazerbato04/Nautilus_Marine_Energy_Harvester/blob/main/assembly/circuit%20with%20AC%20motor.fzz">link </a> <br>
 If you want to see how the circuit change in respond to the input voltage click this <a href="https://github.com/andreazerbato04/Nautilus_Marine_Energy_Harvester/blob/main/assembly/test%20circuit.html">link </a>to see and download the code. You can see how the TP4056 respond on different input voltage from the panel and also from the motor. <br>
<h2>OUR PROTOTYPE</h2>
For our prototype, we put a DC motor for this reason we don't need a bridge rectifier and the capacitor because the voltage is just DC and not AC. Another modification concerns the diode, for space reason is put on the negative line. The other part of the circuit remains unchanged, first line: solar panel, diode, TP4056 second line: DC motor, buck-boost, TP4056 in this order.<br> We built and tested this prototype, confirming it works, in this picture we can see the red led of the first line illuminated, the DC solar panel line, this is because there is sunlight and the panel is charging the batteries. Moreover, if we apply some mechanical torque to the DC motor also the red led of the second line light up.<br>We mounted the project with the glue gun to a cardboard to physically stabilize and secure the project.<br>Finally, we see the solar panel sold with the specific 5V on peak when there is strong sunlight it reach a peak around 6V, since the high voltage can create problems like shut down or loss of efficiency can be a good solution to place a buck converter.<br><br>
...
It is important to analyze the real performance for compare to the energy we want for the sensor and scale the solar panel and the pendulum to achieve the required energy.
